...
# Non-destructive: writes to checkpoints/t2sm/reruns/ours_train_stattn_dualenc_temporalgate_frozen_hilr/ (gitignored).
set -uo pipefail

REPO_ROOT="/mnt/sda/xyh/style-salad"
VENV="${REPO_ROOT}/.venv"
CONFIG="configs/reruns/ours_train_stattn_dualenc_temporalgate_frozen_hilr.yaml"
LOG_DIR="${REPO_ROOT}/artifacts/run_logs"
TRAIN_LOG="${LOG_DIR}/train_stattn_dualenc_temporalgate_frozen_hilr.log"
EVAL_LOG="${LOG_DIR}/evaluate_stattn_dualenc_temporalgate_frozen_hilr.log"
STATUS_FILE="${LOG_DIR}/STATUS_stattn_dualenc_temporalgate_frozen_hilr"
...
echo "[$(date -Is)] START fine-tune (TemporalGate ONLY (frozen backbone), 10x lr+tempo weight, 20 epochs, seed=42, GPU1)" | tee -a "${STATUS_FILE}"
...
python -m style_salad.evaluation.evaluate \
  --config "${CONFIG}" \
  --csv_name metrics_ours_train_stattn_dualenc_temporalgate_frozen_hilr.csv \
  > "${EVAL_LOG}" 2>&1
eval_rc=$?
