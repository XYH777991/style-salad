...
# Non-destructive: writes to checkpoints/t2sm/reruns/ours_train_stattn_dualenc_temporalgate_frozen_tempoonly/ (gitignored).
set -uo pipefail

REPO_ROOT="/mnt/sda/xyh/style-salad"
VENV="${REPO_ROOT}/.venv"
CONFIG="configs/reruns/ours_train_stattn_dualenc_temporalgate_frozen_tempoonly.yaml"
LOG_DIR="${REPO_ROOT}/artifacts/run_logs"
TRAIN_LOG="${LOG_DIR}/train_stattn_dualenc_temporalgate_frozen_tempoonly.log"
EVAL_LOG="${LOG_DIR}/evaluate_stattn_dualenc_temporalgate_frozen_tempoonly.log"
STATUS_FILE="${LOG_DIR}/STATUS_stattn_dualenc_temporalgate_frozen_tempoonly"
...
echo "[$(date -Is)] START fine-tune (TemporalGate ONLY, tempo-only loss (no competing style/supcon), 20 epochs, seed=42, GPU1)" | tee -a "${STATUS_FILE}"
...
python -m style_salad.evaluation.evaluate \
  --config "${CONFIG}" \
  --csv_name metrics_ours_train_stattn_dualenc_temporalgate_frozen_tempoonly.csv \
  > "${EVAL_LOG}" 2>&1
eval_rc=$?
