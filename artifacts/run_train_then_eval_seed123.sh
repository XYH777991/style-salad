#!/usr/bin/env bash
# Same as run_train_then_eval.sh but with random_seed=123 (configs/reruns/ours_train_seed123.yaml)
# to check run-to-run variance against the seed=42 run and the released checkpoint.
# Non-destructive: writes to checkpoints/t2sm/reruns/ours_train_seed123/ (gitignored).
# Started with setsid+nohup+disown so it survives the launching terminal closing.
set -uo pipefail

REPO_ROOT="/mnt/sda/xyh/style-salad"
VENV="${REPO_ROOT}/.venv"
CONFIG="configs/reruns/ours_train_seed123.yaml"
LOG_DIR="${REPO_ROOT}/artifacts/run_logs"
TRAIN_LOG="${LOG_DIR}/train_seed123.log"
EVAL_LOG="${LOG_DIR}/evaluate_seed123.log"
STATUS_FILE="${LOG_DIR}/STATUS_seed123"
...
echo "[$(date -Is)] START train (seed=123, GPU0)" | tee -a "${STATUS_FILE}"
...
python -m style_salad.evaluation.evaluate \
  --config "${CONFIG}" \
  --csv_name metrics_ours_train_seed123.csv \
  > "${EVAL_LOG}" 2>&1
eval_rc=$?
