...
# ours_train_stattn_dualenc_gradscale02.yaml) before committing to the
# other 3 seeds.
# Non-destructive: writes to checkpoints/t2sm/reruns/ours_train_stattn_dualenc_gradscale02/ (gitignored).
set -uo pipefail

REPO_ROOT="/mnt/sda/xyh/style-salad"
VENV="${REPO_ROOT}/.venv"
CONFIG="configs/reruns/ours_train_stattn_dualenc_gradscale02.yaml"
LOG_DIR="${REPO_ROOT}/artifacts/run_logs"
TRAIN_LOG="${LOG_DIR}/train_stattn_dualenc_gradscale02.log"
EVAL_LOG="${LOG_DIR}/evaluate_stattn_dualenc_gradscale02.log"
STATUS_FILE="${LOG_DIR}/STATUS_stattn_dualenc_gradscale02"
...
echo "[$(date -Is)] START train (diffusion_grad_scale=0.2, seed=42, GPU1)" | tee -a "${STATUS_FILE}"
...
python -m style_salad.evaluation.evaluate \
  --config "${CONFIG}" \
  --csv_name metrics_ours_train_stattn_dualenc_gradscale02.csv \
  > "${EVAL_LOG}" 2>&1
eval_rc=$?
