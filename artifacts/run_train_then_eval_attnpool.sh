#!/usr/bin/env bash
# Same as run_train_then_eval.sh (seed=42) but with model.style_encoder.class
# swapped to StyleAttnPool (configs/reruns/ours_train_attnpool.yaml) so it is
# a controlled A/B test against checkpoints/t2sm/reruns/ours_train/ (StyleMLP,
# same seed=42, same everything else).
# Non-destructive: writes to checkpoints/t2sm/reruns/ours_train_attnpool/.
# Started with setsid+nohup+disown so it survives the launching terminal closing.
set -uo pipefail

REPO_ROOT="/mnt/sda/xyh/style-salad"
VENV="${REPO_ROOT}/.venv"
CONFIG="configs/reruns/ours_train_attnpool.yaml"
LOG_DIR="${REPO_ROOT}/artifacts/run_logs"
TRAIN_LOG="${LOG_DIR}/train_attnpool.log"
EVAL_LOG="${LOG_DIR}/evaluate_attnpool.log"
STATUS_FILE="${LOG_DIR}/STATUS_attnpool"
...
echo "[$(date -Is)] START train (StyleAttnPool, seed=42, GPU0)" | tee -a "${STATUS_FILE}"
...
python -m style_salad.evaluation.evaluate \
  --config "${CONFIG}" \
  --csv_name metrics_ours_train_attnpool.csv \
  > "${EVAL_LOG}" 2>&1
eval_rc=$?
