...
class Text2MotionTestDataset(Dataset):
    def __init__(
        ...
        humanml_split_file=None,
        style_split_file=None,
        tiny=False,
        debug=False,
        progress_bar=True,
        **kwargs,
        ...
        if humanml_split_file is None or style_split_file is None:
            if split_file is None:
                raise ValueError(
                    "Provide eval_humanml_split_file and eval_style_split_file, "
                    "or pass legacy split_file for derived 100STYLE split names."
                )
            split_dir = pjoin(os.path.dirname(split_file), "../100style")
            split_base = os.path.basename(split_file).split(".")[0]
            if humanml_split_file is None:
                humanml_split_file = pjoin(split_dir, split_base + "_humanml.txt")
            if style_split_file is None:
                style_split_file = pjoin(split_dir, split_base + "_100STYLE_Filter.txt")

        split_subfile_1 = humanml_split_file
        split_subfile_2 = style_split_file

        motion_to_label = build_dict_from_txt(style_name_dict_path)
        motion_to_style_text = build_dict_from_txt(style_name_dict_path, is_style_text=True)
